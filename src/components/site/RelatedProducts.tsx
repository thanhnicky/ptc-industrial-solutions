    ...
    <div className="mt-12 border-t border-border pt-10">
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((item) => (
          ...
          >
            <div>
              <h4 className="text-sm font-semibold text-ink transition-colors group-hover:text-primary">
                {item.name}
              </h4>
            ...
            <span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-primary">
              Xem chi tiết kỹ thuật <ArrowRight className="size-3 transition-transform group-hover:translate-x-1" aria-hidden="true" />
            </span>
          </Link>
        ))}
      </div>
